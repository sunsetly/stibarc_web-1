function toLink(id,item) {
	try {
		document.getElementById("shitlist").innerHTML = document.getElementById("shitlist").innerHTML.concat('<a style="font-size:100%;text-decoration:none;" href="post.html?id=').concat(id).concat('"><div class="post"><b>').concat(item['title'].replace(/</g, "&lt;").replace(/>/g, "&gt;")).concat('</b><br/>Posted by: <a href="user.html?id=').concat(item['poster']).concat('">').concat(item['poster']).concat("</a></div></a><br/>");
		lastid = id;
	} catch (err) {
		console.log(err);
	}
